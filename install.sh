...
#
#   2. SHORTCUT → `expose-port-cloudflare <port>` from any folder
#      A tiny wrapper is installed at ~/.local/bin/expose-port-cloudflare that
#      execs this repo's scripts/expose-port.sh with the same arguments, so
#      `expose-port-cloudflare 8080` works like running the skill by hand.
...
install_shortcut() {
  ...
  if [ "$DRY_RUN" = 1 ]; then
    say "  • shortcut — [dry-run] would write $wrapper → scripts/expose-port.sh"
  else
    mkdir -p "$bindir"
    printf '#!/usr/bin/env bash\n# %s — CLI shortcut, generated by install.sh (re-run it to refresh)\nexec "%s/scripts/expose-port.sh" "$@"\n' \
...
uninstall_all() {
  ...
  if [ -f "$wrapper" ] && grep -q "scripts/expose-port.sh" "$wrapper" 2>/dev/null; then
    found=1
    if [ "$DRY_RUN" = 1 ]; then
...
say "    expose-port-cloudflare 8080"
say "    expose-port-cloudflare http://localhost:8080"
say ""
say "Stop the tunnel from the repo folder: ./scripts/stop.sh"
if [ "$FAILED" = 1 ]; then
